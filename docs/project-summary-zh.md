...
我的研究围绕一套由四个工作站组成的资产管理流程展开：数据提取与处理、特征工程、模型设计，以及面向客户的实施流程。我在下面保留完整的研究叙述、图表、批注与参考文献，并在需要区分早期判断和当前解释的地方补充研究说明。
...
![数据输入：ASX200top10 股票数据](images/report-p01-Image18.png)

![数据输入：JSON 新闻数据](images/report-p01-Image19.png)

![数据输入：客户资料](images/report-p01-Image20.jpg)

![数据输入：宏观经济指标](images/report-p01-Image21.jpg)
...
![数据输出：股票价格](images/report-p02-Image38.png)

![数据输出：加权收益](images/report-p02-Image39.png)
...
![股票走势特征](images/report-p02-Image40.png)
...
![收益率分布与 QQ 图](images/report-p03-Image43.png)

随后，我进一步检验收益率是否符合正态分布。

![收益率分布检验](images/report-p03-Image44.png)
...
![年龄与风险偏好的分布](images/report-p03-Image45.png)
...
![新闻出现频率](images/report-p04-Image51.jpg)

#### K-means 聚类分析

我使用 K-means 聚类分析，将客户群体分为三个不同的簇。

![K-means 聚类分析](images/report-p04-Image50.jpg)
...
![投资组合的有效前沿](images/report-p06-Image56.png)

##### 3. ARIMA

我使用 ARIMA 预测市场走势（指数）。

![ARIMA 预测](images/report-p06-Image57.png)

##### 4. NLP（与 OLS 模型结合）

我使用 OLS 预测经济指标对市场的影响，并根据经济因素估计当前股价的变化。

> 研究说明：我保留早期报告中“NLP（与 OLS 模型结合）”的标题；其中这段文字实际描述的是宏观经济 OLS 回归。当前实现将宏观回归与 NLP 新闻情绪分析作为两个独立模块，NLP 模块本身不使用 OLS，具体计算见[方法与限制](methodology.md)。

##### 5. 情绪分析

我对已发布的信息进行情绪分析与分类。在模型设计中，我将正面新闻理解为有利于公司股价的信号，将负面新闻理解为可能导致公司股价下跌的信号。（Chowdhury, et al., 2014）

![经济因素引起的权重调整](images/report-p07-Image60.png)

![最终投资权重](images/report-p07-Image61.png)
...
> 研究说明：`0 ≤ weight ≤ 1` 仍允许全部资金配置到一只股票，因此我没有通过这组边界建立真正的分散化限制。并且，当前实现只在基础组合优化时施加这一非负边界，最终加法调整后的权重可能为负，详见[方法与限制](methodology.md)。
